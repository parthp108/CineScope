...
export function MovieProvider({ children }) {
  ...
  const filteredGenre = async () => {
    const data = await fetch(
      `https://api.themoviedb.org/3/discover/movie?with_genres=${activegenre}&api_key=${APIKEY}&page=${page}&include_adult=false`
    );
  ...
  const fetchGenre = async () => {
    const data = await fetch(
      `https://api.themoviedb.org/3/genre/movie/list?api_key=${APIKEY}&language=en-US&include_adult=false`
    );
 ...
 const fetchUpcoming = async () => {
  ...
  while (tempResults.length < 10 && currentPage <= 5) {
    const response = await fetch(
      `https://api.themoviedb.org/3/movie/upcoming?api_key=${APIKEY}&language=en-US&page=${currentPage}&include_adult=false`
    );
    ...
    tempResults = tempResults.concat(filtered);
    currentPage++;
  }

  setUpcoming(upcoming.concat(tempResults));
  setPage(currentPage); // Update your current page state
  setLoader(false);
